...
let serverMainDashboardCache: { payload: MainDashboardPayload; savedAt: number } | null = null;
let serverARDashboardCache: { payload: ARRow[]; savedAt: number } | null = null;

export async function getMainDashboardBootstrap(options?: { forceRefresh?: boolean }): Promise<MainDashboardPayload> {
  if (!MAIN_SHEET_URL) {
    return parseMainRows({});
  }

  const now = Date.now();
  if (!options?.forceRefresh && serverMainDashboardCache && (now - serverMainDashboardCache.savedAt < SHEET_CACHE_TTL_MS)) {
    return serverMainDashboardCache.payload;
  }
  ...
  const payload = parseMainRows(data.table || {});
  serverMainDashboardCache = { payload, savedAt: now };
  return payload;
}

export async function getARDashboardBootstrap(options?: { forceRefresh?: boolean }): Promise<ARRow[]> {
  if (!AR_SHEET_URL) {
    return parseARRows({});
  }

  const now = Date.now();
  if (!options?.forceRefresh && serverARDashboardCache && (now - serverARDashboardCache.savedAt < SHEET_CACHE_TTL_MS)) {
    return serverARDashboardCache.payload;
  }
  ...
  const payload = parseARRows(data.table || {});
  serverARDashboardCache = { payload, savedAt: now };
  return payload;
}
...
const idLocaleNumberFormatter = new Intl.NumberFormat('id-ID');
...
export function formatNumber(num: number) {
  return idLocaleNumberFormatter.format(num);
}
