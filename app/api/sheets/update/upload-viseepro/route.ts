...
function sanitizeViseeproRows(rows: SheetRow[], existingHeaders: string[]) {
  ...
  const employeesHeader = findHeaderByAliases(existingHeaders, [
    'employees total',
    'employee total',
    'jumlah karyawan'
  ]);
...
function remapRowsToExistingHeaders(rows: SheetRow[], existingHeaders: string[]) {
  ...
  const incomingByNormalized = new Map(
    incomingHeaders.map((header) => [normalizeKey(header), header])
  );
  ...
  for (const existingHeader of existingHeaders) {
    ...
    const hasCustomAliases = aliases.some(
      (alias) => normalizeKey(alias) !== normalizeKey(existingHeader)
    );
    if (hasCustomAliases) {
  ...
  return rows.map((row) => {
    ...
    for (const [targetHeader, sourceHeader] of headerMap.entries()) {
      ...
      if (
        (targetValue === undefined || String(targetValue ?? '').trim() === '') &&
...
export async function POST(request: Request) {
  try {
    const viseoproSheetId = resolveSpreadsheetId(
      process.env.VISEEPRO_SHEET_ID,
      process.env.VISEEPRO_SHEET_URL
    );
